...
def app():
    ...
    with st.sidebar.expander(label='Manual', expanded=False):
        ...
        Code：
         [github](https://github.com/su-learning-labo/Payroll_Convert)
        """)

    data_type = st.selectbox('データの種類を選択してください', ['給与', '賞与'])
    uploaded_file = st.file_uploader('経理報告用CSVデータをアップロードしてください', type=['csv'])
    ...
    if uploaded_file is not None:
        ...
        with tab2:
            ...
            post_eom_csv = convert_df_to_csv(df_post_eom)
        ...
        with tab3:
            ...
            st.write('ダウンロード')
            csv_journal = convert_df_to_csv(df_journal)
            st.download_button(
                label='支払仕訳',
                data=csv_journal,
